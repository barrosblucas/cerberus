...
- Toda alteração implementada por IA deve gerar um changelog em `.context/docs/changelog/CHANGELOG_YYYY_MM_DD.md` (um arquivo por dia de mudança), descrevendo objetivo, arquivos tocados e impacto.
...
## Frontend (decisão atual)
- Frontend é **React + Vite** (SPA interna)
- Proibido gerar código React + Vite/SSR/SSG
- Todo fetching é client-side via TanStack Query e `shared/api-client.ts`

## Rotas públicas vs privadas (hard rule)

- Frontend é SPA (React + Vite)
- Rotas públicas vivem em `/public/*`
- Rotas autenticadas vivem em `/app/*`
- Backend expõe:
  - `/v1/public/*` (read-only + rate limit)
  - `/v1/*` (autenticado)
...
## CI scripts de governança
- `pnpm check:file-length` (hard limit 400 + allowlist)
- `pnpm check:prisma-migration` (schema mudou → migration obrigatória)
