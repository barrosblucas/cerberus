                        ...
                        {obras?.map((obra: any) => (
                            <tr key={obra.id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="py-2 px-4">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${obra.status === 'CONCLUIDA' ? 'bg-green-100 text-green-800' :
                                            obra.status === 'PARALISADA' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                                        }`}>
                                        {obra.status}
                                    </span>
                                </td>
                                <td className="py-2 px-4">{new Date(obra.dataInicio).toLocaleDateString()}</td>
                                <td className="py-2 px-4">{new Date(obra.dataPrevisaoTermino).toLocaleDateString()}</td>
                                <td className="py-2 px-4">
