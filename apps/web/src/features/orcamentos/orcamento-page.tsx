...
import {
  ...
  getObra,
  getOrcamento,
} from "../../shared/api-client"; // Ensure getOrcamento is imported
...
interface OrcamentoItem {
  ...
  insumoId?: string;
  composicaoId?: string;
  isComposicao?: boolean; // Flag to show info button
}
...
export function OrcamentoPage() {
  ...
  const loadBudget = async (id: string) => {
    try {
      ...
      // Flatten generic Etapa structure to flat list for MVP
      const flatItems: OrcamentoItem[] = [];
      orcamento.etapas?.forEach((etapa: any) => {
        etapa.itens?.forEach((item: any) => {
          flatItems.push({
            id: crypto.randomUUID(), // New ID for frontend rendering, real ID ignored for now in full sync
            codigo: item.codigo,
                ...
                <td className="px-4 py-2 relative">
                  ...
                  {item.isComposicao && (
                    <button
                      ...
                      className="ml-2 text-blue-600 hover:text-blue-800 text-xs bg-blue-100 px-1 rounded"
