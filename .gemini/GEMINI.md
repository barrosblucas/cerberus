# Gov_SocialLink - Project Context & Agent Guidelines

## 1. Project Overview
**Gov_SocialLink** is a monorepo platform for the Municipality of Bandeirantes/MS, designed to link government services with social data.

### Tech Stack
- **Monorepo Manager:** `pnpm` workspaces + `turborepo`
- **Backend (`apps/api`):** NestJS, Prisma (PostgreSQL), Zod (via contracts)
- **Frontend (`apps/web`):** React, Vite, TanStack Query, Tailwind CSS
- **Shared (`packages/*`):**
  - `contracts`: Single Source of Truth (SSOT) Zod schemas for API and Web.
...
## 3. Project Structure
- `apps/api`: NestJS application. Main business logic.
- `apps/web`: React SPA. Consumes API via `shared/api-client.ts`.
- `packages/contracts`: **CRITICAL**. Defines the API contract (Zod schemas). Changes here propagate to API validation and Web client types.
- `.context/docs`: Documentation and project state.
